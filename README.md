...
Live at: `https://mt-hunting-tag-tracker.wescratty.workers.dev/` (Cloudflare Workers static assets).
...
| `wrangler.jsonc` | Cloudflare Workers static-assets deploy config (`npx wrangler deploy`). |
...
| `assets/` | Fonts, app icons, and the FWP identification-guide illustrations + district map (see "Icons" below). |
| `README.md` | This file. |
...
- **Quick View**: a toggle (top-right, `onQuickViewToggle()`) for once setup is done and you're actually out hunting — hides the branding chrome, hunter profile, district/tag pickers, and everything else down to just the Field Card and its controls, so a phone screen shows nothing but "what can I hunt right now." Remembered per browser/device like the other toggles (`mt-hunt-quickview-v1`). "Exit Quick View" stays visible outside the hidden set so it's always reachable.
- **Where Am I**: a button next to the Field Card controls (`onLocateClick()`) that asks the browser for the phone's GPS location and checks it against a simplified, statewide FWP hunting-district boundary file bundled with the app (`assets/hunting/mt-hunting-districts.geojson` — see "District boundary data" below), using Turf.js's point-in-polygon test. It deliberately does **not** touch `distFilter` or which districts the Field Card shows — it only marks the matching district, wherever it's already displayed (a 📍 next to it in the "Districts shown" panel, a green "you are here" border around its Field Card group), so a hunter's existing filter selections are never silently overridden. Three outcomes: the matched district is one of this app's 13 (highlighted as above); it's a real Montana HD but not one of the 13 this app covers yet (shows "Request HD `{number}` be added" pre-filled with the actual district number — see "Data source & scope"); or the location can't be matched to any district at all (rare — shows a plain error instead of guessing). Also handles permission-denied and no-geolocation-support cases with a clear message rather than failing silently. Nothing about the location is stored or sent anywhere; the match is in-memory only and resets on reload. A short GPS-accuracy caveat (`.ht-locate-caveat`, wrapped in `#locateCaveatSection`) sits under the button by default, hideable with its own "Hide GPS notice" toggle (`mt-hunt-hidelocatecaveat-v1`) — the full version of that same warning is baked into both disclaimer blocks (gate + on-page) regardless, so hiding the short copy doesn't mean the hunter never saw it. **The result and highlight are deliberately temporary**: `scheduleLocateFade()` starts a slow ~30-second fade (`LOCATE_FADE_MS`) the moment a result is shown, then fully clears `currentDistrict` and hides the result once it completes — on purpose, so a hunter who glances at a stale result after walking somewhere else doesn't mistake it for their current position. The fade only touches the highlight *chrome*, never the Field Card content: the result banner fades via `opacity` (it's just text), but `.ht-fc-current`'s border/background fade via their own colors and the "you are here"/📍 text fades via a dedicated `.ht-locate-pin` span — animating `opacity` on the whole `.ht-fc-district` wrapper would take the tag icons down with it, which read as the *tags* disappearing rather than the location guess aging out. A new locate click cancels any fade in progress and starts fresh.
...
**This is not an authoritative legal source.** The disclaimer gate and the persistent privacy note both say so, and this README doesn't either — always confirm against [fwp.mt.gov](https://fwp.mt.gov/hunt/regulations).
...
3. Bump `CACHE_NAME` in `hunting-tags-sw.js` (e.g. `mt-hunting-tag-tracker-v1` → `-v2`) — otherwise returning visitors with a signal stay stuck on last year's cached copy.

## Storage / privacy

Everything is `localStorage`, scoped to whichever browser/device it's opened on — nothing is sent to a server. Keys in use:

- `mt-hunt-tags-v1` — confirmed Deer B/Elk B tags and permits, per district.
- `mt-hunt-filled-v1` — tags marked "filled" (an animal already taken under them), per district.
- `mt-hunt-profile-v1` — age, PTHFV, Bow and Arrow License, General Deer/Elk License.
- `mt-hunt-distfilter-v1` — which districts the Field Card shows.
- `mt-hunt-disclaimer-accepted-v1` — whether the gate has been accepted on this browser.
- `mt-hunt-quickview-v1` — whether Quick View is on.
- `mt-hunt-hidelocatecaveat-v1` — whether the short GPS caveat under "Where Am I" is hidden.
...
`hunting-tags-sw.js` precaches an explicit allowlist of same-origin paths (the page itself, the Rye font, the app icons, the district map image, all icon images, the district boundary GeoJSON, and Turf.js) on first load, then serves them cache-first so the page works with zero connectivity afterward — closed tab, airplane mode, phone restart, etc. Turf.js is vendored locally (`assets/js/turf.min.js`) rather than loaded from a CDN specifically so "Where Am I" doesn't quietly depend on a third-party host being reachable — the whole point of this app is that it keeps working with no signal at all.

**The `/index.html` vs `/` alias** (read this before touching the service worker): Cloudflare's static-asset hosting 307-redirects a request for `/index.html` to the clean canonical URL `/`. Chrome refuses to let a service worker satisfy a top-level navigation with a response reached via a followed redirect — this combination produces `net::ERR_FAILED` for any visitor who types/bookmarks/links the `/index.html` form once the service worker is installed. `hunting-tags-sw.js` handles this by treating `/index.html` and `/` as one resource end to end (`canonicalPath()`), so the fix is already in place from this repo's first commit — this cost real debugging time on the site this app was ported from (a sibling `.html` page under a shared domain hit the exact same bug in production), don't reintroduce it by precaching a literal `.html` path here.

Note: service worker registration could not be verified inside automated preview tooling during development (it blocks SW registration entirely) — verify with a real-browser test (phone or desktop) after any deploy: load with a signal, then reload in airplane mode.
...
Then visit `http://localhost:8000/`. A plain `file://` open works for a quick look, but the service worker and manifest only function over `http(s)://`, and the `/index.html` vs `/` redirect behavior described above doesn't exist locally (that's a Cloudflare-specific production behavior) — a bug in that logic won't show up in local testing, only after a real deploy.

## Deployment

Cloudflare Workers static assets, same pattern as this author's other small standalone tools:

```bash
npx wrangler deploy
```

Reads `wrangler.jsonc` (`name: "mt-hunting-tag-tracker"`). First deploy gets a free `mt-hunting-tag-tracker.<account>.workers.dev` subdomain automatically — no domain purchase needed to start. A real custom domain can be pointed at the same Worker later without changing anything else here.

## Log

- 2026-09-07: Ported out of a larger multi-page site into this standalone repo (git history before this point lives in that other repo, not here). Rewrote all asset paths for a flat single-app layout, replaced the shared site's header/nav/footer with a minimal standalone version, replaced localStorage key prefixes (`wtc-hunting-` → `mt-hunt-`), replaced the parent site's shared branding/logo/favicons with a placeholder mountain icon (real logo/icon design still open, see any open issues), pointed the "request a district" link at this repo's GitHub Issues instead of a contact form that no longer applies here, and updated the service worker's precache list and `.html`-alias handling for the new `/` root layout. App logic, data, and UI behavior are otherwise unchanged from the source.
- 2026-09-08: Added Quick View and "Where Am I". Pulled FWP's statewide hunting-district boundaries (ArcGIS layer 11), simplified with `shapely` at 0.001° tolerance (~90m), and sanity-checked the simplified file against the raw export at three points (Hamilton → HD 260, Wisdom → HD 321, Dillon → HD 324) before shipping it — all three matched. Vendored Turf.js locally instead of pulling it from a CDN so the offline guarantee holds for this feature too. Followed up same day: made the GPS caveat collapsible (with a matching sentence folded into both disclaimer blocks so it's still agreed-to when hidden), and added a ~30-second fade that clears the locate result and highlight automatically so stale location info can't be mistaken for current if glanced at after moving on.
